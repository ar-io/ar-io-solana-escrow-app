 ...
 * DIFFERENT Solana `claimant` (redirecting the asset) or a different
 * asset/amount, and the user would sign it blind (wallets show base58 blobs no
 * human parses). Echoing `initiated.claimant === claimant` is NOT sufficient —
 * only the BYTES are signed, not the JSON envelope.
 *
 * Defence: before signing, LOCALLY rebuild the canonical from what the UI is
 * actually showing (the loaded asset) + the connected source identity + the
 * Solana destination the user typed, and BYTE-COMPARE against the server's
 * bytes. Any mismatch → refuse to sign.
 *
 * The format is byte-pinned to the claims service's `@ar.io/attestor-canonical`
 * builders (`packages/canonical/src/canonical.ts`), which are in turn
 * cross-tested against the on-chain Rust. It is the escrow-claim format WITH the
 * `recipient:` binding line and NO ` v1`/` v2` header suffix. The
 * golden-vector parity test (`canonical-verify.test.ts`) pins our rebuild to the
 * server's exact bytes so a drift breaks the build, not a live claim.
 */
...
export interface ExpectedClaimCanonical {
  /** Network the client PINS (its own `getNetwork()`, never a server echo). */
  network: string;
...
/**
 * Throw unless the server-provided canonical bytes are byte-identical to a
 * locally rebuilt canonical for the asset+claimant+identity the UI is showing.
 * Call this IMMEDIATELY before handing `serverBytes` to the wallet to sign.
 */
export function assertServerCanonicalMatches(
  ...
  if (!bytesEqual(local, serverBytes)) {
    throw new CanonicalMismatchError(
      'The claim server returned a message that does not match this asset and ' +
        'your destination wallet. Refusing to sign — do not proceed and report ' +
        `this. (expected ${local.length} bytes, got ${serverBytes.length})`,
    );
  }
}
