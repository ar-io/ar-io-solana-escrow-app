 ...
 *
 * This is the single backend the app talks to after the F2 service-layer
 * swap (CENTRALIZED_CLAIM_PIVOT_PLAN.md §5). It replaces the on-chain
 * Solana RPC + attestor round-trips: the same claim rules are now enforced
 * server-side, and a valid recipient-key signature IS the authorization to
 * dispense (no client-built Solana transaction).
 *
 ...
 *
 * Deliberately self-contained (fetch + Web Crypto only, no imports from the
 * other service modules) so it is importable in isolation — the escrow
 * client builds on top of it, and a headless integration test drives the
 * exact request/response shapes the UI relies on.
 */
 ...
 * claims-service-not-configured banner, mirroring the old
 * escrow-program-not-configured gate).
 */
...
function requireUrl(): string {
  ...
  if (!url) {
    throw new ClaimsApiError(
      ...
      'Claims service not configured. Set VITE_CLAIMS_API_URL and reload.',
    );
  }
  return url;
}
...
/**
 * Look up all claimable assets for an identity. Provide `recipientId`
 * (b64url(sha256(recipient_pubkey))) OR `protocol`+`address`. A recipient
 * with nothing claimable (or unknown identity) resolves to an empty
 * `assets` list rather than throwing, so the discovery UI shows the
 * "no escrows found" state gracefully.
 */
 ...
 * client-supplied). The wallet must sign `canonicalMessageBytes`.
 */
 ...
 * protocol-specific proof shape the API expects and returns once the proof
 * is verified + the asset consumed (or routed to pending_review).
 *
 * @param signature RSA-PSS (512B) for Arweave, or r||s||v (65B) for Ethereum.
 * @param modulus   Arweave RSA modulus (512B). Bound to the frozen recipient.
 */
...
export async function getClaim(claimId: string): Promise<ClaimStatusView> {
  return request<ClaimStatusView>(
 ...
 * (`confirmed`/`rejected`/`failed`/`expired`) or the timeout elapses.
 * Returns the last observed status. `verified` / `pending_review` /
 * `dispatching` are non-terminal (an on-chain dispatch worker moves them to
 * `confirmed`); the caller decides how to present a still-pending success.
 */
...
// ---------------------------------------------------------------------------
// Encoding helpers (Web-Crypto/Node compatible; no DOM-only APIs)
// ---------------------------------------------------------------------------
...
/** Base64url b64url(sha256(bytes)) — the recipient_id / Arweave-address form. */
export async function deriveRecipientId(bytes: Uint8Array): Promise<string> {
